        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {/* Sunset toggle */}
              <button onClick={toggleSunset} className="p-1.5 rounded-lg hover:bg-slate-700/50 transition text-slate-400 hover:text-amber-400" aria-label="Toggle sunset mode">
                {sunset ? (
          ...
          <div className={`md:hidden absolute top-full left-0 w-full site-header border-b border-slate-800 shadow-2xl overflow-hidden transition-all duration-300 ease-in-out ${
            ...
            <div className="p-4 flex flex-col items-center gap-4">
              ...
              <button onClick={() => { toggleSunset(); setIsMobileMenuOpen(false); }} className="flex items-center gap-2 text-slate-400 hover:text-amber-400 transition-colors">
                {sunset ? (
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
                ) : (
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                )}
                <span className="text-sm">{sunset ? "Dark" : "Sunset"}</span>
              </button>
              {navLinks.map((link) => (
